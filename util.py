...
def get_periods_index_stocks(index, start_date, end_date):
    """
    获取一段时间股票的组成股票代码
    :param index:  指数代码
...
def neutralize(data, date, market_cap, industry_name='sw_l1', country_factor=False):
    """
    中性化，使用行业和市值因子中性化
    input：
...
def draw_acm(data,path,filename,reverse=False,f='9M'):
    ...
    color = ['b', 'g', 'grey', 'c', 'm', 'y', 'k', 'pink','violet', 'brown']
    if reverse:
...
def draw_hedge(data,short,path,filename,reverse=False,f='9M'):
    ...
    :param data: 每期收益dataframe（未累乘）
                index: 日期   columns:分组  value:每组当期收益
    :param short: series index 日期 value空头累积收益率
    :param path: 图片存放路径
    :param filename: 图片名称
...
def Market_value(factor):
    ...
    factor_market = pd.pivot_table(valuation_data, index="date", columns="code", values="market_cap", dropna=False)
